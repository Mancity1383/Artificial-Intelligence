...
in this question the variables are the lessons and the values are the time slots that we give to the classes
the constraint are the the connection between the lessons , for example :   
- courses = ["A", "B", "C"]
- students = {"s1": ["A", "B"], "s2": ["B", "C"]}
- time_slots = [1, 2]

- in here we have connection between A and B s1 took them and we have connection between B and C because s2 took them.
...
class ExamSchedulerCSP:
    ...
    def _backtrack_heuristic(self, assignment, local_domains):
        ...
        for value in local_domains[var]:
            if self.is_consistent(assignment,var,value):
                assignment[var] = value
                result,record_list = self.forward_check(var,value,assignment,local_domains)
                if not result:
                    for var_2,value_2 in record_list:
                            local_domains[var_2].append(value_2)
                else:
                    return self._backtrack_heuristic(assignment,local_domains)
        # For each value:
